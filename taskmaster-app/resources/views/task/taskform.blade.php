    ...
    <form action="{{ isset($task_id) ? route('updateTask', $task_id) : url('/sendTaskForm') }}" method="post">
        ...
        <div class="row mb-3">
            <div class="col-12 col-md-6">
                ...
                <input class="form-control" type="text" id="name" name="name" value="{{ $task_name ?? '' }}" required>
            </div>

            <div class="col-12 col-md-6">
                <label for="type" class="form-label text-light">Tipo:</label>
                <input class="form-control" type="text" id="type" name="type" value="{{ $task_type ?? '' }}" required>
            </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col-12 col-md-6">
                ...
                <input class="form-control" type="date" id="time_limit" name="time_limit" value="{{ $task_time_limit ?? '' }}" min="{{ date('Y-m-d') }}" required>
            </div>
            ...
            <div class="col-12 col-md-6">
                ...
                <select class="form-control text-dark" id="difficulty" name="difficulty" required>
                    <option value="Facil" {{ isset($task_difficulty) && $task_difficulty == 'Facil' ? 'selected' : '' }}>Fácil</option>
                    <option value="Medio" {{ isset($task_difficulty) && $task_difficulty == 'Medio' ? 'selected' : '' }}>Médio</option>
                    <option value="Dificil" {{ isset($task_difficulty) && $task_difficulty == 'Dificil' ? 'selected' : '' }}>Difícil</option>
                </select>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <button class="btn btn-primary" type="submit">{{ isset($task_id) ? 'Atualizar' : 'Cadastrar' }}</button>
            </div>
        </div>
    </form>
</div>
